...
# Tunnel Config Builder (TCB) v4.4
...
- کانفیگ‌های VLESS تولید شده (لینک‌های `vless://`) را در v2rayNG یا کلاینت‌های مشابه وارد کنید.
...
- **JSON برای Sing-box** — کانفیگ کامل برای Sing-box، Hiddify و NekoBox. این فرمت ECH را پشتیبانی نمی‌کند، چون این فیلد در نسخه فعلی Sing-box غیرفعال (Deprecated) شده است.
- **JSON برای Clash / Mihomo** — کانفیگ کامل برای Clash Meta و کلاینت‌های مشابه مانند Clash Verge. این فایل با ساختار JSON نوشته شده اما با پسوند `.yaml` دانلود می‌شود، چرا که JSON زیرمجموعه معتبری از YAML است. توجه داشته باشید که فرمت Clash/Mihomo قابلیت Fragment را پشتیبانی نمی‌کند.
